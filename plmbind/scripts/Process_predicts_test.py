...
for i in range(3):
    with open(args.out_dir + "train_TF_loss_model_test_DNA_train_TF_part_"+str(i+1)+".pkl", 'rb') as f:
        preds = pickle.load(f)
...
with open(args.out_dir + "train_TF_loss_model_test_DNA_val_TF.pkl", 'rb') as f:
    preds = pickle.load(f)
...
for i in range(3):
    with open(args.out_dir + "val_TF_loss_model_test_DNA_train_TF_part_"+str(i+1)+".pkl", 'rb') as f:
        preds = pickle.load(f)
    AUROC = MultilabelAUROC(num_labels=len(train_TF_splits[i]), average='none')
    AverageP = MultilabelAveragePrecision(num_labels=len(train_TF_splits[i]), average='none')
...
with open(args.out_dir + "val_TF_loss_model_test_DNA_val_TF.pkl", 'rb') as f:
    preds = pickle.load(f)
AUROC = MultilabelAUROC(num_labels=len(val_TFs), average='none')
AverageP = MultilabelAveragePrecision(num_labels=len(val_TFs), average='none')
...
output_df.to_csv(args.out_dir+"Test_Results.csv")